        ...
        function ($scope, $element, $attrs, $compile, $parse, $document, $timeout, $uibPosition, dateFilter, uibDateParser, uiDatetimePickerConfig, $rootScope) {
            ...
            this.init = function (_ngModel) {
                ...
                var popupEl = angular.element('' +
                    ...
                    '<div uib-datepicker style="margin:0 auto;display: inline-block">' +
                    '</div>' +
                    '<div uib-datepicker style="margin:0 20px;display: inline-block"></div>' +
                    '</div>');
                if (ngModelOptions) {
                ...
                if (!$scope.datepickerOption) {
                    $scope.datepickerOption = {};
                }
                timepickerEl.attr('datepicker-options', 'datepickerOption');
                ...
                angular.forEach(['minDate', 'maxDate', 'initDate'], function (key) {
                    if ($scope.datepickerOptions[key]) {
                        if (key == 'minDate') {
                            timepickerEl.attr('min', 'datepickerOption.minDate');
                        } else if (key == 'maxDate')
                            timepickerEl.attr('max', 'datepickerOption.maxDate');
                    }
                });
            ...
            $scope.dateSelection = function (dt, opt) {
                ...
                var date = $scope.date ? dateFilter(dt, dateFormat) : null;
                $element.val(date);
